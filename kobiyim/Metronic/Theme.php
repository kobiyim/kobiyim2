...
class Theme
{
    ...
    {
        if (file_exists(asset('media/icons/'.$path))) {
            return sprintf('<span class="%s">%s</span>', $classNames, file_get_contents(asset('media/icons/'.$path)));
        }
    ...
    function getIcon($name, $class = '', $type = '', $tag = 'span')
    {
        ...
        if ($type === 'duotone') {
            $icons = cache()->remember('duotone-icons', 3600, function () {
                return json_decode(file_get_contents(asset('icons.json')), true);
            });
