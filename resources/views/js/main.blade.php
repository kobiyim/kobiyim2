		...
		function loadModal(dataa = {}, open = null) {
			if(open == true) {
				if ($("#" + dataa.key).length == 1) {
					$("#" + dataa.key).replaceWith(postData('{{ route('modals') }}', dataa, true).data);
				} else {
					$("#modals").append(postData('{{ route('modals') }}', dataa, true).data);
				}
				openModal(dataa.key);
			} else if(open != null) {
				if ($("#" + open).length == 1) {
					$("#" + open).replaceWith(postData('{{ route('modals') }}', dataa, true).data);
				} else {
					$("#modals").append(postData('{{ route('modals') }}', dataa, true).data);
				}
				openModal(open);
